...
class RoomOp(QMainWindow, Ui_MainWindow):
    ...
    def showRoom(self,rid,rtype,rstorey,rprice,rdesc,rpic,endtime,i,j):
        ...
        self.glayout.setContentsMargins(10,3,10,3)#设置边距
        # 下面展示信息
        self.flayout = QVBoxLayout()#垂直布局
        self.glayout.addLayout(self.flayout,i,j)
        lb = QLabel(self)
        lb.setFixedSize(150,80)
        lb.setPixmap(QPixmap(rpic))#图片转QT格式
        lb.setStyleSheet("border:1px solid white")
        lb.setScaledContents(True)#图片自适应大小
        self.flayout.addWidget(lb)
        # self.flayout.addStretch(1)
    ...
    def findRoom(self):
        ...
        if length / 3 == 0:
            ...
            ll = length/3
        self.scrollAreaWidgetContents.setGeometry(QtCore.QRect(0, 0, 580,341*ll))
        self.gridLayoutWidget.setGeometry(QtCore.QRect(0, 0, 601, 341*ll))

        for i in range(1 + int(length / 3)):#行
            for j in range(3):
                if k == length:
                    break
                print(k)
