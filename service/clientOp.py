...
class ClientOp(QMainWindow, Ui_MainWindow):
    def __init__(self,parent=None):
        ...
        self.listWidget.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)#设置垂直滚动条
        self.listWidget.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)#设置水平滚动条
        self.listWidget.currentRowChanged.connect(self.stackedWidget.setCurrentIndex)#多页面切换
        self.searchNB.clicked.connect(self.searchClient)
        self.commitAdd.clicked.connect(self.addClient)
        # self.commitAddT.licked.connect(self.tableDel)
    ...
    #删除客户
        ...
        #     type = '团队'
            ...
            QMessageBox().information(None, "提示", "必须选择一种方式！", QMessageBox.Yes)
            return False
        row_selected = self.tableClient.selectedItems()#放回长度是你选择的目标的长度
        # print("len row_selected = ",len(row_selected))
        if len(row_selected) == 0:
            return
        row = row_selected[0].text()
        c = Client()
        c.tableDelDB(type,row)#都选中第一个姓名
